...
const TableThirtySix = ({ addresses, address, dateFrom, dateTo, orientation, logo, reportDetails }) => {
  const [reportData, setReportData] = useState([])

  useEffect(() => {
    const fetchTableThirtySixReport = async () => {
      const { data } = await axios.get(`/api/table_thirtySix/${address}/${dateFrom}/${dateTo}`)
      if(data.success){
        setReportData(data.data)
      }
    }

    fetchTableThirtySixReport()
  }, [])
    ...
    <div className={`bg-white py-8 ${orientation}`}>
      ...
      <div className='grid place-items-center'>
          <table className='report_table'>
            <thead>
              <tr>
                <th>Primary Needs of the Barangay</th>
