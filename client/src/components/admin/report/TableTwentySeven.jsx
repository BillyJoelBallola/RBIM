...
const TableTwentySeven = ({ addresses, address, dateFrom, dateTo, orientation, logo, reportDetails }) => {
  const [reportData, setReportData] = useState([])

  useEffect(() => {
    const fetchTableTwentySevenReport = async () => {
      const { data } = await axios.get(`/api/table_twentySeven/${address}/${dateFrom}/${dateTo}`)
      if(data.success){
        setReportData(data.data)
      }
    }

    fetchTableTwentySevenReport()
  }, [])
...
export default TableTwentySeven