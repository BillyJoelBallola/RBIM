...
const TableTwentyFour = ({ addresses, address, dateFrom, dateTo, orientation, logo, reportDetails }) => {
  const [reportData, setReportData] = useState([])

  useEffect(() => {
    const fetchTableTwentyFourReport = async () => {
      const { data } = await axios.get(`/api/table_twentyFour/${address}/${dateFrom}/${dateTo}`)
      if(data.success){
        setReportData(data.data)
      }
    }

    fetchTableTwentyFourReport()
  }, [])
...
export default TableTwentyFour